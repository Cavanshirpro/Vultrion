...
def find_window_by_pid_windows(pid: int):
    if not IS_WINDOWS:
        return None
...
def find_window_by_pid_x11(pid: int):
    """
    Linux/X11 için PID -> XID bulmaya çalışır.
    python-xlib yoksa veya Wayland ise None döner.
    """
    if not IS_LINUX:
        return None
...
def find_native_window_by_pid(pid: int):
    if IS_WINDOWS:
        return find_window_by_pid_windows(pid)
    if IS_LINUX:
        return find_window_by_pid_x11(pid)
    return None
...
class ScrcpyDock(QDockWidget):
    def __init__(
        ...
        scrcpy_program: str,
        *,
        capture_orientation_lock: bool = True,
        ...
        self.status = QLabel("Scrcpy başlatılıyor...")
    ...
    def build_args(self):
        ...
        # Telefon döndüğünde görünümün bozulmaması için:
        # scrcpy docs: --capture-orientation=@ locks to the initial orientation.
        if self.capture_orientation_lock:
            args.append("--capture-orientation=@")

        # İstersen sabit ekran yönü:
        if self.display_orientation is not None:
            args.append(f"--display-orientation={self.display_orientation}")

        # Ek bir display rotasyonu:
    ...
    def start(self):
        ...
        if not self.process.waitForStarted(5000):
            raise RuntimeError(f"scrcpy başlatılamadı: {self.scrcpy_program}")

        self.attach_timer.start()
        self.health_timer.start()
        self.status.setText("Pencere bekleniyor...")

    def try_attach(self):
        if self._closing or self._attached:
            return
        ...
        if qwindow is None:
            self.status.setText("Bu platformda foreign window embedding desteklenmiyor.")
            return
        ...
        # Pencere dekorasyonunu azaltmak için Windows'ta ek stil temizliği
        if IS_WINDOWS:
            try:
            ...
            try:
                user32.ShowWindow(native_id, SW_SHOW)
            except Exception:
                pass
    ...
    def ensure_alive(self):
        ...
        if self._attached:
            if self.scrcpy_hwnd is None:
                return
            if IS_WINDOWS and user32 and not user32.IsWindow(self.scrcpy_hwnd):
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Scrcpy Embedded Dock")
        self.resize(1280, 800)

        # Windows: ./adb/scrcpy.exe
        # Linux/macOS: scrcpy yolunu gerekirse değiştir.
        if IS_WINDOWS:
            program = "./adb/scrcpy.exe"
        else:
            program = "scrcpy"

        self.dock = ScrcpyDock(
            program,
            capture_orientation_lock=True,  # cihaz dönse de görüntü bozulmasın
            display_orientation=None,       # istersen "90" verebilirsin
            angle=None,                     # istersen 90 verebilirsin
        )
        self.addDockWidget(Qt.DockWidgetArea.LeftDockWidgetArea, self.dock)

        QTimer.singleShot(0, self.dock.start)
